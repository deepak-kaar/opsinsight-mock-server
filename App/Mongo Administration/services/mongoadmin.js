...
const get_collections = async function (req, res) {
  ...
  try {
    ...
    // Filter to only include allowed collections
...
const create_document = async (req, res) => {
  ...
  try {
    ...
    if (collection === "DataBase") {
      ...
      req.body.queryId = objectId.toString();
    }
    if (collection === "Instance") {
      const newEntityObjectId = new ObjectId();
      req.body._id = newEntityObjectId;
      req.body.instanceId = newEntityObjectId.toString();
    }
    if (collection === "Organization") {
      if (req.body.appName) {
        const appsCol = db.collection("Apps");
        const app = await appsCol.findOne({ appName: req.body.appName });
        if (app) {
          req.body.appId = app._id.toString();
        }
        else{
      ...
      req.body._id = orgObjectId;
      req.body.orgId = orgObjectId.toString();
    }
    if (collection === "Apps") {
      if (!req.body.appName || !req.body.appDescription) {
        return res.status(400).json({
          token: "400",
          response: "Validation failed",
          errors: ["appName and appDescription are required"],
        });
      }
      const newObjectId = new ObjectId();
      req.body = {
        _id: newObjectId,
        appId: newObjectId.toString(),
        ...
        appLogo: req.file ? req.file.buffer : null,
        appLogoName: req.file ? req.file.originalname : null,
        appLogoType: req.file ? req.file.mimetype : null,
        appStatus: "active",
        createdOn: new Date(),
      };
    }
    if (collection === "Entity Data") {
      const newObjectId = new ObjectId();
      req.body._id = newObjectId;
      req.body.entityOrInstanceId = req.body.dataId;
    }
    ...
    }, LoggerService.MODULES.MONGODB_ADMIN);
...
const schemaMap = {
  ...
  "Entity Data": entity_data_collection
};
...
const create_document_mapping = async (req, res) => {
  ...
  try {
    ...
    let lookupData = {};
    if (schema.lookupFields) {
      for (const [field, lookup] of Object.entries(schema.lookupFields)) {
        const lookupCol = db.collection(lookup.collection);
        const data = await lookupCol.find({}).toArray();
        lookupData[field] = data.map(item => item[lookup.field]);
      }
    }
...
async function getSummary(req, res) {
  try {
    ...
    const moduleFilter = {
      module: { $ne: "MongoDBAdmin" }
    };

    const total = await logs.countDocuments(moduleFilter);
...
async function getCategory(req, res) {
  try {
    ...
    // ✅ Fetch ONLY MongoDBAdmin logs for ApplicationLogs
    const filter =
