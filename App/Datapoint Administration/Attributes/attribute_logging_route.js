...
// Logging endpoints for attribute search
...
async function getCategory(req, res) {
    try {
        ...
        // Filter logs by ATTRIBUTE_SEARCH module
        const data = await db.collection(category)
            .find({ module: "ATTRIBUTE_SEARCH" })
            .sort({ timestamp: -1 })
            .toArray();
...
async function getSummary(req, res) {
    try {
        ...
        // Get logs filtered by ATTRIBUTE_SEARCH module
        const logs = db.collection("ApplicationLogs");

        const total = await logs.countDocuments({ module: "ATTRIBUTE_SEARCH" });
        const errors = await db.collection("ErrorLogs").countDocuments({ module: "ATTRIBUTE_SEARCH" });
        const security = await db.collection("SecurityEvents").countDocuments({ module: "ATTRIBUTE_SEARCH" });
        const audit = await db.collection("AuditTrail").countDocuments({ module: "ATTRIBUTE_SEARCH" });

        // --- Top Users Aggregation ---
        const topUsersAgg = await logs.aggregate([
            { $match: { module: "ATTRIBUTE_SEARCH" } },
            {
                $group: {
                    _id: "$user.username",
                    count: { $sum: 1 }
