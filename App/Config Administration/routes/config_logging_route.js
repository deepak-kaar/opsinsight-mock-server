...
router.get('/category/:category', async (req, res) => {
  try {
    ...
    const logs = await db.collection(collectionName)
      .find({ module: 'Config Administration' })
      .sort({ timestamp: -1 })
      .limit(1000)
      .toArray();
...
router.get('/logger/summary', async (req, res) => {
  try {
    ...
    const [appLogs, auditLogs, errorLogs, securityLogs, perfLogs] = await Promise.all([
      db.collection('ApplicationLogs').countDocuments({ module: 'Config Administration' }),
      db.collection('AuditTrail').countDocuments({ module: 'Config Administration' }),
      db.collection('ErrorLogs').countDocuments({ module: 'Config Administration' }),
      db.collection('SecurityEvents').countDocuments({ module: 'Config Administration' }),
      db.collection('PerformanceMetrics').countDocuments({ module: 'Config Administration' })
    ]);
